...
<section class="abouts content content-page" id="content">
	<div class="column-left role-tabs side-tabs" id="abouts_tabs">
		<div class="right">
			...
			<nav>
				<?php if(is_array($category)): $k = 0; $__LIST__ = $category;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$category): $mod = ($k % 2 );++$k;?><a href="<?php echo ($category["url"]); ?>" <?php if(($k) == "1"): ?>class="on tab-on"<?php endif; ?> ><?php echo ($category["title"]); ?></a><?php endforeach; endif; else: echo "" ;endif; ?>
				<div class="follower" style="top: 106px;"></div>
			</nav>
		</div>
	</div>
	<div class="column-right" id="abouts_contents">
		<?php if(is_array($opRes)): $k = 0; $__LIST__ = $opRes;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$opRes): $mod = ($k % 2 );++$k;?><div class="tab-content <?php if(($k) != "1"): ?>row-col2 hidden<?php endif; ?>">
			<h1>
                <?php echo ($opRes["title"]); ?>
                <small><?php echo ($opRes["etitle"]); ?></small>
			</h1>
			<div class="<?php if(($k) == "1"): ?>row-col2<?php else: ?>row pre<?php endif; ?>">
				<?php echo ($opRes["Body"]); ?>
			</div>
