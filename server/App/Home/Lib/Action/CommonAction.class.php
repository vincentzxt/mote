...
class CommonAction extends Action{
    ...
    public function _initialize() {
		// 获取公共站点参数
		$zconfig = M('SysInfo');
		$dataConfig = $zconfig->find(1);
		// dump($dataConfig);
		$this->assign('web',$dataConfig);

        $map['status'] = array('eq',1);
		$map['id'] = array('neq',5);
        // 获取尾部网站地图信息
        $mapres = $this->array_listtree_func(0,'category',$map);
		// file_put_contents('e:\www\liang\tiaoshi.ss1',print_r($mapres,1),8);
        // dump($mapres);
        $this->assign('mapres',$mapres);

		// 获取合作伙伴信息
		$dao1 = M('link');
		$map1['status'] = array('eq',1);
		$hzres = $dao1->where($map1)->select();
		$this->assign('hz',$hzres);

		$map = array();
		$map['level'] = array('lt',3);
		$map['id'] = array('neq',5);
		$dhlist = $this->array_listtree_func(0,"category",$map);
		...
		$onurl = MODULE_NAME.'/'.ACTION_NAME;

		$this->assign('onurl',$onurl);
	...
	public function tophead($id){
		$dao =M('category');
		$tophead = $dao->where('pid = '.$id)->select();
    ...
    +----------------------------------------------------------
     */
    protected function _listJson($model, $map, $sortBy = '', $asc = false,$pk=0) {
