...
class RBAC {
    ...
    static function saveAccessList($authId=null)
    {
        ...
        // 对管理员开发所有权限
		//dump($_SESSION[C('ADMIN_AUTH_KEY')]);
        if(C('USER_AUTH_TYPE') !=2 && !$_SESSION[C('ADMIN_AUTH_KEY')] )
		//dump($authId);
        $_SESSION['_ACCESS_LIST']	=	RBAC::getAccessList($authId);
		//dump($_SESSION[C('_ACCESS_LIST')]);
        return ;
    }
	...
	static public function getModuleAccessList($authId,$module) {
        ...
        $sql    =   "select access.node_id from ".
                    ...
                    $table['user']." as user,".
                    $table['access']." as access ".
                    "where user.id='{$authId}' and user.role_id=role.id and ( access.role_id=role.id  or (access.role_id=role.pid and role.pid!=0 ) ) and role.status=1 and  access.module='{$module}' ";
        $rs =   $db->query($sql);
        $access	=	array();
        foreach ($rs as $node){
            $access[]	=	$node['node_id'];
